...
struct Student {
    var name : String
    var favoriteTeacherIDproperty : Int
}
...
struct School {
    var name : String
    var listOfTeachers : [Teacher]
    
    func getTeacher(withID : Int) -> Teacher? {
        for teacher in listOfTeachers  {
            if teacher.id == withID {
                return teacher
            }
        }
        return nil
    ...
    func printFavoriteTeacherInfo(student: Student) {
        if let favorite = getTeacher(withID : student.favoriteTeacherIDproperty) {
            print(favorite.name)
            print(favorite.id)
            
            if let email = favorite.email {
                print(email)
            }
            
            if let hairColor = favorite.hairColor {
                print(hairColor)
            }
            
        } else {
            print("Teacher not in this school")
        }
    }
}
 ...
 */
var student1 = Student(name: "student", favoriteTeacherIDproperty: 1)
var student2 = Student(name: "student", favoriteTeacherIDproperty: 4)
var teacher1 = Teacher(name : "teacher1", id: 1)
var teacher2 = Teacher(name : "teacher2", id: 2, email : "teacher@gmail.com", hairColor: "Green")
var teacher3 = Teacher(name : "teacher2", id: 3, email : nil, hairColor: nil)
let school = School(name: "school", listOfTeachers: [teacher1, teacher2])
school.printFavoriteTeacherInfo(student: student1)
school.printFavoriteTeacherInfo(student: student2)
